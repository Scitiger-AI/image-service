...
class FileUtils:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url) as response:
                    ...
                    if file_type == "image" and not content_type.startswith('image/'):
                        if not cls.is_likely_image_from_url(url):
                            logger.warning(f"可能不是图像文件: {url}, Content-Type: {content_type}")
    ...
    @staticmethod
    def is_likely_image_from_url(url: str) -> bool:
        """根据URL判断是否可能是图像文件"""
        ...
        try:
            ...
            if mime_type:
                if mime_type.startswith('image/'):
                    ...
                    return ext if ext else ".png"  # 默认使用.png
            return None
        except:
            return None
    ...
    def convert_path_to_urls(cls, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 处理images字段中的本地路径
        if "images" in result_copy and isinstance(result_copy["images"], list):
